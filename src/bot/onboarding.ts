...
import { onboardingTurn, type OnboardProfile, type OnboardInputPart, type OnboardTurn } from "../llm/onboard.js";
...
// One turn of conversational onboarding. `parts` is text or voice audio.
export async function handleOnboarding(ctx: Context, user: User, parts: OnboardInputPart[]) {
  ...
  try {
    ...
    await ctx.reply("Had a hiccup there — say that again?");
    return;
  }

  // Record what the user said (all reads/writes scoped to this user's id only).
  ...
  // If the model says "confirmed" without a live proposal, we simply relay its
  // message and stay in 'chat' — nothing gets committed without a real proposal.
...
// Persist only the fields the model learned this turn, with defensive range checks
// (a chatty model could return junk; code is the gatekeeper).
function applyUpdates(userId: number, u: OnboardTurn["updates"]) {
  ...
  if (u.activity && ["sedentary", "light", "moderate", "active"].includes(u.activity)) set("activity", u.activity);
...
// ---- per-user conversation log (reuses chat_log; always scoped by user_id) ----
