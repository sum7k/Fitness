...
A voice-first **Telegram fitness bot** ("Sized", name TBD). Users speak/type/photograph what they ate or did; a cheap multimodal LLM extracts structured entries and assigns **rough calorie estimates**. The product bet: *adherence beats accuracy* — a rough log kept for a year beats a perfect log abandoned in three weeks. Read `SPEC.md` for original product rationale (it still describes the older t-shirt-size UX) and `ARCHITECTURE.md` for the V1 design. **Live code speaks calories**, not XS–XXXL.
...
2. `extract()` (`src/llm/extract.ts`) — **single** cheap-model call returns `{transcript, intent, entries[], weight_kg, chat_text}` as strict JSON-schema output. Transcription + intent routing + entry extraction + kcal estimation all happen in this one call.
3. `resolveKcal()` (`src/domain/kcal.ts`) — **waterfall**: personal `overrides` → global `kcal_cache` → trust the LLM's kcal (and cache it, but only unambiguous unquantified names).
4. Insert entries, reply one message per entry with inline kcal-adjustment buttons (±50/±100), then a tally bar.
...
**Calories are the user-facing currency** (`src/domain/tally.ts`): entries show `~N kcal`; daily budget and remaining are kcal. Internally `users.daily_budget_units` still stores a scaled form (1 unit ≈ 200 kcal) via `energy.ts`. Exercise credit is damped to 50% (`EXERCISE_CREDIT` in `src/domain/credit.ts`). Tally sums `kcal_estimate`. Streaks count *days with any log*, not days on budget.
...
- Callback data is compact: `e:<entryId>:k:<delta>` (±50/±100) and `e:<entryId>:del`, parsed by regex in `bot.ts`. A kcal correction also upserts a personal `override` so the item is remembered next time.
